...
(function( $ ) {
    ...
    $.fn.extend({
		...
		*/
        diyUpload:function( opt, serverCallBack ) {
			...
			if( opt.delfile ) {
				var delfileCallBack = opt.delfile;
				delete opt.delfile;
			}
			...
			if( opt.defaultBox ) {
				if(JSON.stringify(opt.defaultBox) != "{}"){
					defaultBox($fileInput,opt.defaultBox,opt);
				}
			...
			webUploader.on('uploadSuccess',function( file, response ){
				...
				$diyBar.fadeOut( 1000 ,function(){
					...
					$fileBox.children('.diySuccess').one('click',function(){
						removeLi($(this).parent('li'), file.id, webUploader );	
						if ( delfileCallBack ) {
							delfileCallBack(response['id']);
						}
					});
	...
	//默认已上传文件
	function defaultBox( $fileInput,data,opt) {
		...
		$.each( data,function( key, row ){
			//添加子容器;
			var li = '<li class="diyUploadHover"> \
						<div class="viewThumb"><img src="../'+ row['path'] + row['name'] +'" ></div> \
						<div class="diySuccess DiydefaultDel"></div> \
						<input name="id_Del" type="hidden" value="'+ row['id'] +'" /> \
						<input name="_csrf_Del" type="hidden" value="'+ row['_csrf'] +'" /> \
						<div class="diyFileName">'+ row['name'] +'</div>\
					</li>';
			$parentFileBox.children('.fileBoxUl').append( li );
		});
		
		$('.DiydefaultDel').show();
		$('.DiydefaultDel').one('click',function(){
			if( opt.deldefault ) {//删除默认
				opt.deldefault($(this).parent('li').find('[name=id_Del]').val(),$(this).parent('li').find('[name=_csrf_Del]').val());
			}	
			if ( $(this).parent('li').siblings('li').length <= 0 ) {
				$(this).parent('li').parents('.parentFileBox').remove();
			} else {
