...
<article class="page-container">
                            <?php $form =  ActiveForm::begin(
									...
									<?= $form->field($model, 'exa_user_id')
										->textInput(['class' => 'input-text', 'placeholder' => '用户试卷ID'])?>
									<?= $form->field($model, 'uid')
										->textInput(['class' => 'input-text', 'placeholder' => '回答人'])?>
									<?= $form->field($model, 'subject_id')
										->textInput(['class' => 'input-text', 'placeholder' => '题目ID'])?>
									<?= $form->field($model, 'content')
										->textInput(['class' => 'input-text', 'placeholder' => '内容与回答'])?>
									<?= $form->field($model, 'score')
										->textInput(['class' => 'input-text', 'placeholder' => '得分'])?>
									<?= $form->field($model, 'is_error')
										->textInput(['class' => 'input-text','placeholder' => '是否正确回答'])?>
                                    <?= $form->field($model, 'teacher_uid')
                                        ->textInput(['class' => 'input-text','placeholder' => '批题教师ID'])?>
                                    <?= $form->field($model, 'teacher_reply')
                                        ->textInput(['class' => 'input-text','placeholder' => '教师回复'])?>
                                     <!-- 状态 下拉控件 -->
                                    <?= $form->field($model, 'status')
                                        ->dropDownList($model->attributeValues()['status'], ['class' => 'select-box']) ?>

                                    <?= $form->field($model, 'create_date')
                                        ->textInput(['class' => 'input-text','placeholder' => '回答时间'])?>
