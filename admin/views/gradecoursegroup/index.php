...
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> <?=$this->context->userRole->name?> <span class="c-gray en">&gt;</span> 年级课程组 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
...
<div class="text-l"><?=$searchViews;?></div>
	...
	<table class="table table-border table-bordered table-hover table-bg">
		...
		<thead>
			<tr>
				<th scope="col" colspan="12">年级课程组</th> 
			</tr>
			<tr class="text-c">
                ...
                <th><?= $ClassModel->getAttributeLabel('course_com_str') ?></th>
        ...
        <?php foreach($model as $k=>$v){?>
			<tr class="text-c">
				...
				<?php
                    ...
                    echo '<td>' . $v->course_com_str . '</td>';     
                    echo '<td>' . $v->type_id . '</td>';  
                    echo '<td><a href='.Url::to(['update', 'id' => $v->id]).'>' . $v->enumeration('status',$v->status) . '</a></td>';          
                    echo ' <td>';
