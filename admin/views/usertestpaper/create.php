...
<article class="page-container">
					...
					<?= $form->field($model, 'exa_id')
                        ->textInput(['class' => 'input-text', 'placeholder' => '试卷ID']) ?>
                     <?= $form->field($model, 'uid')
                        ->textInput(['class' => 'input-text', 'placeholder' => '回答人']) ?>
                    <?= $form->field($model, 'content')
                        ->textInput(['class' => 'input-text', 'placeholder' => '内容与回答[JSON数据]']) ?>
                    <?= $form->field($model, 'score')
                        ->textInput(['class' => 'input-text', 'placeholder' => '总得分']) ?>
                     <?= $form->field($model, 'right_num')
                        ->textInput(['class' => 'input-text', 'placeholder' => '正确数']) ?>
                     <?= $form->field($model, 'error_num')
                        ->textInput(['class' => 'input-text', 'placeholder' => '错误数']) ?>
                        <!-- 日期表 -->
                        ...
                        <div class="row cl">
                         ...
                         <label class="col-md-offset-1  col-sm-3 taglabel">
                            <input checked type="checkbox"
                             ...
                             .value==1?0:1)" ><input type="hidden" id='temp' name="ExaUser[status]" value="1">
                            <span class="lbl"></span>
                        </label>
                            <span class="lbl"></span>
                        </label>
