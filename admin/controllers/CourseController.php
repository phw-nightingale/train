...
class CourseController extends BaseController
{
    ...
    {
        $this->ClassModel = new \common\models\Course;
    }
    ...
    public function actionCreate()
    {
        ...
        if ($ClassModel->load(Yii::$app->request->post()) && $ClassModel->validate()) {
            ...
            $all= \common\models\SchoolRole::find()->where(['status'=>1])->asArray()->all();//
            $dataList['name']=array_column($all,'name','id');
            $user=\common\models\SchoolUser::find()->where(['status'=>1])->asArray()->all();
            $username['username']=array_column($user,'username','id');
            $type=\common\models\CourseType::find()->asArray()->all();
            $courseType['name']=array_column($type,'name','id');
            return $this->render($this->actionID, [
                'ClassModel' => $this->ClassModel,
                'model' => $ClassModel,
                'dataList'=>$dataList,
