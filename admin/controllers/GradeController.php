...
class GradeController extends BaseController
{
    ...
    {
        $this->ClassModel = new \common\models\GradeClass;
    }
    /**
     * 列表
     * @return string
     */
    public function actionGradeClass()
    {
		// echo 11;die;
    ...
    public function actionCreate()
    {
        ...
        if ($ClassModel->load(Yii::$app->request->post()) && $ClassModel->validate()) {
            ...
            Yii::$app->response->format = Response::FORMAT_JSON;  //设置返回json格式
            if ($ClassModel->save()) {
            ...
            $data = SchoolInfo::find()->select(['id','name'])->asArray()->all();
            $dataList['schoolinfo'] = array_column($data,'name','id');
            
            return $this->render($this->actionID, ['ClassModel' => $this->ClassModel,'model' => $ClassModel,'dataList'=>$dataList]);
        }
    }
    ...
    public function actionDelete()
    {
        ...
        Yii::$app->response->format = Response::FORMAT_JSON;
		if(empty($cbox)) {
			return ['code' => 400, 'message' => '请选择需要删除的数据'];
		}
